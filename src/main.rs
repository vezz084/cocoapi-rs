...
fn main() -> Result<()> {
    ...
    // dbg!(coco.get_images_from_ids(&[139, 0]));
    // dbg!(coco.get_annotations_from_ids(&[20000000, 0]));
    // dbg!(coco.get_categories_from_ids(&[180, 92, 0]));

    // dbg!(coco.get_possible_annotations_from_image_ids(&[]));

    // dbg!(coco.get_possible_annotations_from_category_ids(&[]));
    // dbg!(coco.get_possible_annotations_from_category_ids(&[92]));
    // dbg!(
    //     coco.get_possible_annotations_from_category_ids(&[92, 0, 93])
    //         .unwrap()
    //         .len()
    // );
    // //ids(&[139, 0]));
    ...
    //         .len()
    // );
    // dbg!(coco.get_possible_annotations_from_filters(None, None));
    // dbg!(coco.get_possible_annotations_from_filters(None, Some(&[])));
    // dbg!(coco.get_possible_annotations_from_filters(Some(&[]), None));
    // dbg!(coco.get_possible_annotations_from_filters(Some(&[]), Some(&[])));
    // //
    // dbg!(coco.get_possible_annotations_from_filters(None, Some(&[0])));
    // dbg!(
    //     coco.get_possible_annotations_from_filters(None, Some(&[0, 92]))
    //         .unwrap()
    //         .len()
    // );

    // dbg!(
    //     coco.get_possible_annotations_from_filters(None, Some(&[0, 92, 93]))
    //         .unwrap()
    //         .len()
    // );

    // dbg!(coco.get_possible_annotations_from_filters(Some(&[0]), None));
    // dbg!(
    //     coco.get_possible_annotations_from_filters(Some(&[0, 8211]), None)
    //         .unwrap()
    //         .len()
    // );
    ...
    // dbg!(coco.get_possible_annotations_from_filters(Some(&[0, 92, 8211]), Some(&[92, 96, 0])));

    // dbg!(coco.get_possible_annotations_from_filters(Some(&[0, 92, 8211]), Some(&[0])));

    dbg!(coco.get_possible_images_from_filters(None, None));
    dbg!(coco.get_possible_images_from_filters(None, Some(&[])));
    dbg!(coco.get_possible_images_from_filters(Some(&[]), None));
    dbg!(coco.get_possible_images_from_filters(Some(&[]), Some(&[])));
    //
    dbg!(coco.get_possible_images_from_filters(None, Some(&[0])));
    dbg!(
        coco.get_possible_images_from_filters(None, Some(&[0, 92]))
            .unwrap()
            .len()
    );

    dbg!(
        coco.get_possible_images_from_filters(None, Some(&[0, 92, 93]))
            .unwrap()
            .len()
    );

    dbg!(coco.get_possible_images_from_filters(Some(&[0]), None));
    dbg!(
        coco.get_possible_images_from_filters(Some(&[0, 8211]), None)
            .unwrap()
            .len()
    );

    dbg!(
        coco.get_possible_images_from_filters(Some(&[0, 92, 8211]), None)
            .unwrap()
            .len()
    );
    ...
    dbg!(coco.get_possible_images_from_filters(Some(&[0, 92, 8211]), Some(&[92, 96, 0])));

    dbg!(coco.get_possible_images_from_filters(Some(&[0, 92, 8211]), Some(&[0])));
